...
1. 仅 `workflowVersion` 为 `1`、`2`、`3` 的 checkpoint 被支持。未知版本不得按旧图或 V3 猜测执行；必须 fail-closed，返回稳定错误并保留诊断信息，不得产生文件、命令、Git、审批或调度副作用。
2. Runtime `tool-result` 必须先在 API 边界校验当前 pending `callId` 与 `toolName`。完全一致的已消费结果允许幂等 `200` 返回；任何不匹配、篡改或冲突重放返回 `409 Conflict`，并且不得改变 checkpoint、scheduler 或任务状态。
3. checkpoint 必须先成功持久化，之后才允许同步 scheduler。保存失败时不得释放或推进任何并发槽位；恢复时以持久化 checkpoint 作为唯一事实来源。
4. V3 模型网关调用不得持有全局 `_tasks_lock`。同一 task 使用串行保护、checkpoint 版本比较或租约避免竞争；模型调用在全局锁外执行，结果写回前必须重新校验对应 checkpoint/lease。
5. V3 必须有明确的并发配额。若沿用现有 scheduler，V3 进入模型回合前应取得任务槽位，终态后在 checkpoint 成功保存后释放；不得绕过 `max_concurrent_tasks`。
6. `ToolResultRequest` 在 HTTP 边界必须递归验证 JSON 值、有限数字、字符串 artifact refs 与标识长度；非法输入返回 `422`，而不是通过运行时异常把任务推进为人工介入。

这些不变量优先于旧的“V3 暂不使用 scheduler”过渡行为；过渡实现不得暴露为对外 API 的不一致状态。
